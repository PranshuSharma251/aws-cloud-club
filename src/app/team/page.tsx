...
const faculty = [
  { 
    ...
    image: "/images/dr.jpg", 
    // ✅ Changed to ResearchGate specific key
...
const vicePresidents = [
  { 
    name: "Hemant Dadhich", 
    role: "Vice President", 
    // ✅ Empty space holder (keeps alignment perfect)
    customTag: "\u00A0", 
...
const getRoleStyle = (category: string) => {
  switch(category) {
    case 'tech': return { color: '#00E5FF', icon: 'ph:code-bold' }; 
    case 'creative': return { color: '#E91E63', icon: 'ph:paint-brush-broad-bold' }; 
    case 'management': return { color: '#FF9900', icon: 'ph:chart-bar-bold' }; 
    default: return { color: '#8A63D2', icon: 'ph:user-bold' };
  }
};

// 1. LEADERSHIP CARD COMPONENT
const LeadershipCard = ({ member, color = '#8A63D2' }: { member: any, color?: string }) => (
  <Box sx={{
    position: 'relative',
    borderRadius: 4,
    overflow: 'hidden',
    ...
    <Box sx={{ position: 'relative', height: 340, width: '100%', bgcolor: '#0a0510' }}>
      ...
      {/* Social Reveal */}
      <Box className="social-overlay" sx={{
        position: 'absolute', bottom: 0, left: 0, right: 0,
        p: 2, display: 'flex', justifyContent: 'center', zIndex: 2,
        opacity: 0, transform: 'translateY(20px)',
        transition: 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)'
      }}>
        {/* ✅ LOGIC: Show LinkedIn OR ResearchGate icon */}
        {member.linkedin ? (
          <IconButton 
            component="a"
          ...
          >
           ...
           <IconButton 
            ...
            sx={{ 
              bgcolor: 'white', 
              boxShadow: '0 0 15px rgba(0,0,0,0.5)',
              '&:hover': { bgcolor: '#00ccbb', transform: 'scale(1.1)' } // ResearchGate Green/Blue color
            }}
          >
            {/* ✅ ResearchGate Icon */}
    ...
    {/* Text Area */}
    <Box sx={{ p: 3, textAlign: 'center', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
      <Typography variant="h5" fontWeight="bold" sx={{ color: 'white', mb: 0.5 }}>{member.name}</Typography>
      
      {/* Role Title */}
      <Typography variant="subtitle2" fontWeight="bold" sx={{ color: color, textTransform: 'uppercase', letterSpacing: 1, mb: 0.5 }}>
        {member.role}
      </Typography>

      {/* ✅ NEW: Custom Tag (e.g. SDE Intern, Captain) */}
      {member.customTag && (
        <Typography 
            variant="caption" 
            sx={{ 
      ...
      <Typography variant="body2" sx={{ color: 'grey.400', lineHeight: 1.6, fontSize: '0.95rem' }}>{member.bio}</Typography>
    </Box>
  </Box>
);
...
const CouncilCard = ({ member }: { member: any }) => {
  ...
  return (
    <Box sx={{
      ...
      <Box>
        <Typography variant="h6" fontWeight="bold" sx={{ color: 'white', fontSize: '1rem', lineHeight: 1.2 }}>{member.name}</Typography>
        <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 0.5, fontSize: '0.75rem' }}>{member.role}</Typography>
      </Box>
      <Icon icon="ph:hash-bold" style={{ position: 'absolute', right: -10, bottom: -10, fontSize: '60px', color: 'rgba(255,255,255,0.02)', transform: 'rotate(-15deg)' }} />
    </Box>
  );
}
...
export default function TeamPage() {
  return (
    <Box sx={{ bgcolor: '#0f0518', minHeight: '100vh', position: 'relative', overflow: 'hidden', pb: 20 }}>
      ...
      {/* Background Blobs */}
      <Box sx={{ position: 'absolute', top: '-10%', left: '20%', width: '800px', height: '800px', background: 'radial-gradient(circle, rgba(138, 99, 210, 0.1) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(100px)', zIndex: 0 }} />
      <Box sx={{ position: 'absolute', top: '40%', right: '-10%', width: '600px', height: '600px', background: 'radial-gradient(circle, rgba(255, 153, 0, 0.08) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(100px)', zIndex: 0 }} />
      ...
      <Container maxWidth="xl" sx={{ position: 'relative', zIndex: 1, px: { xs: 2, md: 6 }, pt: 10 }}>
        ...
        {/* ================= 1. FOUNDERS ================= */}
        <Box sx={{ mb: 10 }}>
           <Typography variant="h4" fontWeight="bold" sx={{ color: 'white', mb: 5, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
        ...
        <Box sx={{ mb: 10 }}>
           <Typography variant="h4" fontWeight="bold" sx={{ color: 'white', mb: 5, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
              <Icon icon="ph:chalkboard-teacher-fill" color="#FFF" /> Faculty Advisor
           </Typography>
        ...
        <Box>
           <Box sx={{ textAlign: 'center', mb: 8 }}>
             ...
             <Box sx={{ width: 60, height: 4, bgcolor: '#FF9900', mx: 'auto', borderRadius: 2, mt: 2 }} />
           </Box>
